...
local_conf = diffusion_conf['local']
prefix_prompt = display(diffusion_conf['prefix-prompt'])
default_neg_prompt = display(diffusion_conf['default-neg-prompt'])
model_id = display(local_conf['model-id'])
i2i_support = display(local_conf['i2i-support'])
image_max_width = display(local_conf['image-max-width'])
image_max_height = display(local_conf['image-max-height'])
# 创建调度器
scheduler = None
...
def inference(prompt, guidance, steps, width=512, height=512, seed=0, img=None,
              strength=0.5, neg_prompt="", auto_prefix=False) -> (Image, str):
    ...
    try:
        if i2i_support and img is not None:
            return _img2img(prompt, neg_prompt, img, strength, guidance, steps, generator), None
        else:
            return _txt2img(prompt, neg_prompt, guidance, steps, width, height, generator), None
    except Exception as e:
        print(e)
        return None, str(e)


def _txt2img(prompt, neg_prompt, guidance, steps, width, height, generator) -> Image:
    with lock:
        # 构建通道对象函数
...
def _img2img(prompt, neg_prompt, img, strength, guidance, steps, generator) -> Image:
    ratio = 1
