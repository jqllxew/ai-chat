...
def receive():
    ...
    if req_json.get('post_type') == 'request':
        ...
        if request_type == "friend":
            print(f"[qq]好友申请:{uid} 验证信息:{comment}")
            handle_friend_add(flag, "true")
        if request_type == "group":
            sub_type = req_json.get('sub_type')
            gid = req_json.get('group_id')
            if sub_type == "add":
                print(f"[qq]加群申请:{uid} 验证信息:{comment}")
            elif sub_type == "invite":
                print(f"[qq]邀请进群:{uid} 群号:{gid}")
...
def send_private(uid, message):
    ...
    if res.get('status') != "ok":
        print(f"[qq]send_private err:{res}")
...
def handle_friend_add(flag, approve):
    ...
    if res["status"] != "ok":
        print(f"[qq]handle_friend_add err:{res}")
...
def handle_group_invite(flag, approve):
    ...
    if res["status"] != "ok":
        print(f"[qq]handle_group_invite err:{res}")
